    ...
    if(opt == 3)
    {
        ...
        if(opt >= 1 and opt <= 2)
        {
            if(self->askYesNo("所以你的第二次轉職決定成為#b" .. jobName[opt] .. "#k嗎？在決定之後你再也無法反悔重新選擇...你真的確定嗎？") == 1 and (inventory->exchange(0, 4031012, -1) == 0))
            {
                ...
                qr->setState(7500, 1, "");
                self->say("那從現在開始你就是#b" ..jobName[opt] .. "了#k。永遠不要停止履行盜賊使命的腳步...總有一天，我還會讓你變得更加強大的！");
            }
            else
...
if(nJob == 0) --1st job advancement
{
    ...
    {
        if(self->askYesNo("看來你有充分的天賦與資質，你想成為#r盜賊#k嗎？") == 1)
        {
            self->sayNext("這是最後的機會，一旦決定了就永遠無法改變了。");
            if(self->askYesNo("你確定真的想成為#r盜賊#k？") == 1)
            {
                target->setJob(400);
                target->incSP(1 + (target->getLevel() - 10) * 3);
                inventory->exchange(0, 1472000, 1);
                inventory->exchange(0, 2070000, 1);
                self->say("希望你在未來的盜賊道路上能夠順利，祝福你！");
            }
            else
        ...
        self->say("我可以看出你的潛能，但目前你還需多加努力，有朝一日才有可能成為#r盜賊#k。");
}
else if(math.floor(nJob / 10) == 40) --2nd job advancement
{
    if(target->getLevel() < 30)
        self->say("成為盜賊之後過的還好嗎？什麼？你想要成為更強大的盜賊？嗯...目前看起來你尚未有資格成為#r刺客#k或者#r俠盜#k。");
    else
    {
        qrVal = qr->get(7500);
        if(qrVal == "s")
        {
            ...
            else if(inventory->itemCount(4031011) == 0)
            {
                self->say("看來你把信封給弄丟了對吧！我可以再給你一次機會，請把信封給保管好！");
                if(inventory->exchange(0, 4031011, 1) != 0) 
                    self->say("請確保背包的其他欄有足夠的欄位。");
            }
            else
                self->say("你還沒去找#b盜賊轉職教官#k嗎？請把握時間把信封交給他並完成他所交代的任務再回來找我。");
        }
        else if(self->askYesNo("嗯...看來這段時間你進步的非常的快，看起來與之前相當不同。當時你看起來非常的弱小，現在我可以向你保證，你已經有盜賊該有的樣子...那你覺得呢？你想不想要獲得更強大的力量？通過一些簡單的考驗我就能幫你...你準備好了嗎？") == 1)
        {
            self->sayNext("非常好...看起來你非常有勇氣，但我需要親自測試看看是不是真的。這個考驗並不會太難，你應該能夠通過的。首先，先把這封信帶在身上，記住不要弄不見了。");
            if(inventory->exchange(0, 4031011, 1) == 0) 
            {
                qr->setState(7500, 1, "s");
                self->sayNext("請把這封信交給位於#r北方工地#k的#b盜賊轉職教官#k。他在那邊進行轉職的指導工作...把這封信交給他然後他會把你帶到測試地點。詳細的說明將由他告訴你。");
            }            
            else
                self->say("請確保背包的其他欄有足夠的欄位。");
        }
...
else if((nJob == 410 or nJob == 420) and target->getLevel() >= 70) --3rd job advancement
    job3rdMissionStart(2020011, "猴子沼澤地");