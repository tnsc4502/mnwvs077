...
if(field and field->getID() == 0) then
    if(self->askYesNo("歡迎來到楓之谷世界！這裡的訓練所是為了要幫助初心者～我會建議您利用訓練所來熟悉遊戲，當然您也可以直接略過並開始冒險，是否要進入訓練場呢？") == 1) then
        self->say("我將送您進入，但是在那之前還是要提醒您，請務必遵循指導員的指示，祝您好運～")
        target->transferField(1, "")
    else
        if(self->askYesNo("您是否真的要直接開始旅程呢？") == 1) then
