...
// WvsCenter_GUIDlg.cpp : implementation file
//
...
static CWvsCenter_GUIDlg* g_pConsoleDialog = nullptr;
...
CWvsCenter_GUIDlg::CWvsCenter_GUIDlg(CWnd* pParent /*=NULL*/)
	: CDialogEx(IDD_WVSCENTER_GUI_DIALOG, pParent)
{
	m_hIcon = AfxGetApp()->LoadIcon(IDR_MAINFRAME);
}

void CWvsCenter_GUIDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialogEx::DoDataExchange(pDX);
...
BEGIN_MESSAGE_MAP(CWvsCenter_GUIDlg, CDialogEx)
	...
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_EN_CHANGE(IDC_EDIT1, &CWvsCenter_GUIDlg::OnEnChangeEdit1)
END_MESSAGE_MAP()
...
// CWvsCenter_GUIDlg message handlers

BOOL CWvsCenter_GUIDlg::OnInitDialog()
{
	/*
...
void CWvsCenter_GUIDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
void CWvsCenter_GUIDlg::OnPaint()
{
	if (IsIconic())
	{
...
HCURSOR CWvsCenter_GUIDlg::OnQueryDragIcon()
{
...
void CWvsCenter_GUIDlg::OnEnChangeEdit1()
{
}
...
*/
BOOL CWvsCenter_GUIDlg::PreTranslateMessage(MSG* pMsg)
{
	if (WM_KEYDOWN == pMsg->message && VK_RETURN == pMsg->wParam)
	{
		if (GetFocus() == GetDlgItem(IDC_EDIT1))
		{
			...
			*/
			...
			if (s == "Clear")
				m_ConsoleOutput.SetWindowTextW(L"");
...
void CWvsCenter_GUIDlg::AppendMessage(int nLogLevel, const std::string & sMsg)
{
