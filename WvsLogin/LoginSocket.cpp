...
{
	OutPacket oPacket;
	oPacket.Encode2(LoginSendPacketType::Client_ClientStartResponse);
	oPacket.Encode4(0);
	SendPacket(&oPacket);
}
...
void LoginSocket::OnLoginBackgroundRequest()
{
	...
	OutPacket oPacket;
	oPacket.Encode2(LoginSendPacketType::Client_ClientLoginBackgroundResponse);
...
void LoginSocket::OnCheckPasswordRequst(InPacket *iPacket)
{
	...
	if (nCheckResult == LoginResult::res_PasswdCheck_Success)
	{
		if ((pEntry = WvsBase::GetInstance<WvsLogin>()->GetLoginEntryByAccountID(m_pLoginEntry->nAccountID)))
		{
			...
			if (!WvsBase::GetInstance<WvsLogin>()->GetSocket(pEntry->uLoginSocketSN))
			{
				...
				OutPacket oPacket;
				oPacket.Encode2(CenterRequestPacketType::RequestLoginAuth);
	...
	LOGIN_SUCCESS:
		...
		{
			oPacket.Reset();
			oPacket.Encode2(LoginSendPacketType::Client_RequestSelectGenderAnd2ndPassword);
...
void LoginSocket::OnSelectGenderAnd2ndPassword(InPacket *iPacket)
{
	...
	OutPacket oPacket;
	oPacket.Encode2(LoginSendPacketType::Client_SelectGenderAnd2ndPasswordResult);
...
void LoginSocket::OnCheckWorldStatusRequst(InPacket *iPacket)
{
	...
	{
		OutPacket oPacket;
		oPacket.Encode2((short)LoginSendPacketType::Client_WorldStatusResult);
...
void LoginSocket::SendWorldInformation()
{
	...
	for (int i = 0; i < nCenterCount; ++i)
	{
		...
		{
			OutPacket oPacket;
			oPacket.Encode2(LoginSendPacketType::Client_WorldInformationResponse);
...
void LoginSocket::OnClientSelectWorld(InPacket *iPacket)
{
	...
	if (pCenter->CheckSocketStatus(SocketBase::SocketStatus::eConnected))
	{
		...
		OutPacket oPacket;
		oPacket.Encode2(CenterRequestPacketType::RequestCharacterList);
...
{
	OutPacket oPacket;
	oPacket.Encode2(LoginSendPacketType::Client_ClientSecondPasswordResult);
...
void LoginSocket::OnClientCheckDuplicatedID(InPacket *iPacket)
{
	...
	OutPacket oPacket;
	oPacket.Encode2(CenterRequestPacketType::RequestCheckDuplicatedID);
	oPacket.Encode4(GetSocketID());
	oPacket.Encode4(m_pLoginEntry->nAccountID);
...
void LoginSocket::OnClientCreateNewCharacter(InPacket *iPacket)
{
	...
	OutPacket oPacket;
	oPacket.Encode2(CenterRequestPacketType::RequestCreateNewCharacter);
	oPacket.Encode4(GetSocketID());
	oPacket.Encode4(m_pLoginEntry->nAccountID);
...
void LoginSocket::OnClientSelectCharacter(InPacket *iPacket)
{
	...
	OutPacket oPacket;
	oPacket.Encode2(CenterRequestPacketType::RequestGameServerInfo);
	oPacket.Encode4(GetSocketID());
	oPacket.Encode4(m_pLoginEntry->nAccountID);
