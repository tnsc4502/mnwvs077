...
// WvsGame_GUIDlg.cpp : implementation file
//

#include "stdafx.h"
#include "WvsGame_GUI.h"
#include "WvsGame_GUIDlg.h"
...
static CWvsGame_GUIDlg* g_pConsoleDialog = nullptr;
...
CWvsGame_GUIDlg::CWvsGame_GUIDlg(CWnd* pParent /*=NULL*/)
	: CDialogEx(IDD_WvsGame_GUI_DIALOG, pParent)
{
	m_hIcon = AfxGetApp()->LoadIcon(IDR_MAINFRAME);
}

void CWvsGame_GUIDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialogEx::DoDataExchange(pDX);
	...
	DDX_Control(pDX, IDC_STATIC1, m_CommandLogLabel);
	DDX_Control(pDX, IDC_STATIC2, m_ServerLogLabel);
	DDX_Control(pDX, IDC_RICHEDIT22, m_CommandLog);
}

BEGIN_MESSAGE_MAP(CWvsGame_GUIDlg, CDialogEx)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_EN_CHANGE(IDC_EDIT1, &CWvsGame_GUIDlg::OnEnChangeEdit1)
END_MESSAGE_MAP()
...
// CWvsGame_GUIDlg message handlers

BOOL CWvsGame_GUIDlg::OnInitDialog()
{
	/*
	...
	*/
	...
	std::thread(GameApp::InitializeService, __argc, pArg).detach();
...
void CWvsGame_GUIDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
HCURSOR CWvsGame_GUIDlg::OnQueryDragIcon()
{
...
void CWvsGame_GUIDlg::OnEnChangeEdit1()
{
}
...
*/
BOOL CWvsGame_GUIDlg::PreTranslateMessage(MSG* pMsg)
{
	if (WM_KEYDOWN == pMsg->message && VK_RETURN == pMsg->wParam)
	{
		if (GetFocus() == GetDlgItem(IDC_EDIT1))
		{
			...
			*/
			...
			GameApp::OnCommandPromptInput(s);
			m_CommandInput.SetWindowTextW(L"");

			//MessageBox(sCommandInput);
			UpdateData(FALSE);
		}
		return true;
	}
...
void CWvsGame_GUIDlg::AppendMessage(int nLogLevel, const std::string & sMsg)
{
